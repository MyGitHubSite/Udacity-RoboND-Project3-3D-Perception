...
# Exercise-2 TODOs:
    ...
    pcl.save(cloud, '01_point_cloud.pcd')

    # Statistical Outlier Filtering
    fil = cloud.make_statistical_outlier_filter()
    fil.set_mean_k(10)   #5
    fil.set_std_dev_mul_thresh(0.5)   #0.5
    cloud_filtered = fil.filter()
    pcl.save(cloud_filtered, '02_outliers_filtered.pcd')
    ...
    pcl.save(cloud_filtered, '04_passthrough.pcd')
    ...
    pcl.save(cloud_table, '05_cloud_table.pcd')
    pcl.save(cloud_objects, '06_cloud_objects.pcd')
    ...
    ec.set_ClusterTolerance(0.05)   #0.15   #0.01
    ec.set_MinClusterSize(50)       #20     #20
    ec.set_MaxClusterSize(5000)   #3000   #2000
...
def pr2_mover(object_list):     
    ...
    for pick in pick_list_param:
        ...
        pick_pose = Pose()
        pick_pose.position.x = np.asscalar(centroids[0])   #convert from numpy.float to python float type
        pick_pose.position.y = np.asscalar(centroids[1])   #convert from numpy.float to python float type
        pick_pose.position.z = np.asscalar(centroids[2])   #convert from numpy.float to python float type
    ...
    send_to_yaml(yaml_filename, labels)
